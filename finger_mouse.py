    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for num, hand in enumerate(results.multi_hand_landmarks):
                ...
                pyautogui.moveTo(
                    hand.landmark[8].x * screen_width, hand.landmark[8].y * screen_height)
                ...
                dis_fingers_scroll = np.linalg.norm(
        ...
        if just_clicked == True:
            image = cv2.rectangle(image, (int(image.shape[1] * 0.35), int(image.shape[0] * 0.75)), (int(
                image.shape[1] * 0.60), int(image.shape[0] * 1)), (0, 255, 0), 4)

        else:
            image = cv2.rectangle(image, (int(image.shape[1] * 0.35), int(image.shape[0] * 0.75)), (int(
                image.shape[1] * 0.60), int(image.shape[0] * 1)), (0, 0, 0), 4)
